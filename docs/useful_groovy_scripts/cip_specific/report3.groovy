...
def parseDirName(dir) {
    if(!dir.getName().endsWith("_CT") && !dir.getName().endsWith("_CXR") &&
       !dir.getName().endsWith("_ct") && !dir.getName().endsWith("_cxr")) {
        return null;
    }
    else {
...
def comparePatientId(dir1, dir2) {
   ...
   def patientName = parseDirName(dir1)
   if(patientIds.size() > 1) {
       println "!!!!!!!!!!!!!!!!!patient:"+patientName+" has patient ids that dont agree across subdirectories"
   }
   else {
       println "patient: "+patientName+" agrees"
   }
