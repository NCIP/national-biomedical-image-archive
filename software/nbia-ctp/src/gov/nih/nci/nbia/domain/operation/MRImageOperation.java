 ...
 *
 */
public class MRImageOperation extends DomainOperation implements MRImageOperationInterface{
	       ...
	       // mri = (MRImage) this.update(hql, mri);
	        ...
	        if(ret != null && ret.size() > 0) {
	        	if(ret.size() == 1) {
	        		...
	        		throw new Exception("mr_image table has duplicate records, please contact Data Team to fix data, then upload data again");
	        	}
	        }
     ...
     */
    private static void populateMRImageFromNumbers(Map numbers, 
    		                                       MRImage mri) throws Exception {
		String temp;
        ...
        if ((temp = (String) numbers.get(
                        DicomConstants.MAGNETIC_FIELD_STRENGTH)) != null) {
